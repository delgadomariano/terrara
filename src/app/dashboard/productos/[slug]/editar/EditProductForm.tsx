...
export default function EditProductForm({ product }: { product: any }) {
  // Combine thumbnail and other images in order
  const initialImages = [];
  ...
  const initialVideoUrl = product.videos && product.videos.length > 0 ? product.videos[0].url : '';
  ...
  return (
    <form action={updateProduct} className="glass-panel" style={{ padding: '32px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ padding: '24px', backgroundColor: '#f9fafb', borderRadius: '8px', border: '1px dashed var(--border-color)' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '16px' }}>
          La primera foto será la miniatura del catálogo.
        </p>
      ...
      {images.length === 0 && <p style={{ color: '#ef4444', fontSize: '14px' }}>Sube al menos una foto para guardar.</p>}
    </form>
  );
}
